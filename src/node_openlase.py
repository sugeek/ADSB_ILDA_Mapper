...
# --- OSC CLIENT SETUP ---
# Establish communication with the OpenLase software
try:
...
logging.info("OpenLase Safety Node initialized and entering loop.")
...
while True:
    ...
    try:
            ...
            for ac in aircraft_data:
                if "lat" in ac:
                    ...
                    if slant_range <= config["hazard_zone"]["max_nohd_meters"]:
                        in_zone = True
                        logging.warning(f"OpenLase Hazard Detected: Aircraft at {slant_range:.1f}m")
                        break
        
        # --- WATCHDOG & ACTUATION LOGIC ---
        # Trigger blanking if aircraft in zone OR telemetry watchdog expires
        ...
        logging.error(f"OpenLase telemetry request failed: {e}")
        osc_client.send_message("/openlase/blank", 1)
        
    except Exception as e:
        # Catch-all for any other runtime errors
        logging.error(f"OpenLase runtime loop error: {e}")
        osc_client.send_message("/openlase/blank", 1)
